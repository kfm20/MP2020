...
colnames(Upstream1_MinDO)[2] <- "Upstream1_MinDO"
colnames(Impoundment1_MinDO)[2] <- "Impoundment1_MinDO"

Up1andImp_MinDO<-cbind(Upstream1_MinDO, Impoundment1_MinDO)
Up1andImp_MinDO <- left_join(Upstream1_MinDO,Impoundment1_MinDO,by="Date")
...
Up1andImp_MinDO_Diff<-Up1andImp_MinDO%>%
  mutate(Diff= Upstream1_MinDO-Impoundment1_MinDO)

#Save this dataset
write.csv(Up1andImp_MinDO_Diff, row.names = FALSE, file =("./Processed/MilburnieDam_Up1andImp_MinDO_Diff_processed.csv"))
```

```{r Separate all MinDO and Diff data into before and after removal }
Up1andImp_MinDO_Diff_Before <- Up1andImp_MinDO_Diff %>% filter(Date < as.Date("2017-09-21"))

Up1andImp_MinDO_Diff_After <- Up1andImp_MinDO_Diff %>% filter(Date > as.Date("2017-09-21"))


Up1andImp_MinDO_Diff_After$TimeStamp<-"After"
Up1andImp_MinDO_Diff_Before$TimeStamp<-"Before"

#Save these
write.csv(Up1andImp_MinDO_Diff_After, row.names = FALSE, file =("./Processed/MilburnieDam_Up1andImp_MinDO_Diff_After_processed.csv"))
write.csv(Up1andImp_MinDO_Diff_Before, row.names = FALSE, file =("./Processed/MilburnieDam_Up1andImp_MinDO_Diff_Before_processed.csv"))
...
```{r Graph DODiff}
MinDODiff_Before_plot <-ggplot(Up1andImp_MinDO_Diff_Before, aes(x = Date, y = Diff)) +
geom_line(size=.75, color="royalblue4")   +ylab("Daily Minimum DO Difference (mg/L)")+ xlab("Sampling Date")+scale_x_date(date_breaks = "1 year", date_minor_breaks= "months", date_labels = "%m/%y")+
  labs(title = "Avg Minimum DO Difference Before Removal")+
  theme(panel.background = element_rect(fill = "grey97"),
        panel.grid.major = element_line(color = "gray68", size = 0.5),
        panel.grid.minor = element_line(color = "gray85", size = 0.25))+   
    geom_vline(data=d, mapping=aes(xintercept=date), color="red", lty=5, size=.5)
...
MinDODiff_After_plot <-ggplot(Up1andImp_MinDO_Diff_After, aes(x = Date, y = Diff)) +
geom_line(size=.75, color="royalblue4")   +ylab("Daily Minimum DO Difference (mg/L)")+ xlab("Sampling Date")+scale_x_date(date_breaks = "1 year", date_minor_breaks= "months", date_labels = "%m/%y")+
  labs(title = "Avg Minimum DO Difference After Removal")+
  theme(panel.background = element_rect(fill = "grey97"),
        panel.grid.major = element_line(color = "gray68", size = 0.5),
        panel.grid.minor = element_line(color = "gray85", size = 0.25))+   
    geom_vline(data=d, mapping=aes(xintercept=date), color="red", lty=5, size=.5)
...
geom_point(size=.75) + ylim(0, 15)+ ylab("")+
 xlab("Sampling Date")+
  ...
  theme(panel.background = element_rect(fill = "grey97"),
        panel.grid.major = element_line(color = "gray68", size = 0.5),
        panel.grid.minor = element_line(color = "gray85", size = 0.25), plot.title = element_text(size = 10), axis.title.y = element_text(size = 12), axis.title.x = element_text(size = 12), legend.position = "bottom", legend.title = element_blank(), aspect.ratio=4/16)
...
#Combine
MinDO_RepresentativeSites_CombinedPlot<-plot_grid(RepSites_MinDO_Before_plot2,RepSites_MinDO_After_plot2, 
...
```{r Remake difference graphs}
#Recalculate difference graphs and then graph and combine with MinDO_RepresentativeSites_CombinedPlot
#then..Can you manipulate so there is one legend not two??
