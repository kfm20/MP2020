...
#Graph
removal_vline <- as.Date(c("2017-09-21"))  
removal_vline <- which(Up1AvgDO$Date %in% removal_vline)

Up1plot <-ggplot(Up1AvgDO, aes(x = Date, y = AvgDO)) +
geom_point(size=.75, color="royalblue4")  + ylim(0, 15) +ylab("Avg Daily DO (mg/L)")+ xlab("Sampling Date")+scale_x_date(date_breaks = "1 year", date_labels = "%Y")+
  ...
  geom_vline(xintercept = as.numeric(Up1AvgDO$Date[removal_vline]),
             col = "red", lwd = .5)
